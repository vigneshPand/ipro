...
const LeaveHistoryScreen = ({ navigation }) => {
    ...
    // Fetch history when History tab becomes active
    const loadHistory = useCallback(async () => {
        try {
            const user = await AuthService.getUserInfo();
            if (user?.userId) {
                const { fromDate, toDate } = getMonthRange();
                await fetchHistoryLeaves(user.userId, new Date().getFullYear(), fromDate, toDate);
            }
        } catch (err) {
            console.error(err);
        }
    ...
    }, [activeTab, fetchPendingLeaves, fetchHistoryLeaves]);
