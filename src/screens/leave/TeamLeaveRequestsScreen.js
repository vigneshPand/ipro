...
const TeamLeaveRequestsScreen = ({ navigation }) => {
    ...
    useFocusEffect(
        useCallback(() => {
            // Only fetch without search on initial focus or filter/tab changes
            // Search triggers are handled separately via handleSearchChange
            if (!searchQuery) {
                fetchData('');
            }
    ...
    useEffect(() => {
        ...
        // Reset lists when filters change to show loading state
